...
function Volumes({
  ...
  const volumesQuery = useQuery({
    ...
    queryFn: () => getVolumes(pageSize, page),
  })

  if (volumesQuery.isLoading) {
    return (
      <ul className="volumes">
        {[...Array(pageSize)].map((_it, i) => (
          <li key={i} className="skeleton-wrapper">
            <div
              className="skeleton skeleton-image"
              style={{
                height: `${randomIntFromInterval(minThumbnailHeight, maxThumbnailHeight)}px`,
              }}
            />
            <div className="title skeleton skeleton-text" />
            <button className="add-to-cart">
              Add to Cart <img src={iconShoppingCart} />
            </button>
          </li>
        ))}
      </ul>
